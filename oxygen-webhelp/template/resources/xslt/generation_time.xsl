    ...
    <xsl:template match="*:div[contains(@class, 'footer-container')]" mode="copy_template">
        ...
        <xsl:if test="oxyf:getParameter('webhelp.footer.add.generation.time') = 'yes'">
            <div class="generation_time">
                Generation date: <xsl:value-of select="format-dateTime(current-dateTime(), '[D01].[M01].[Y0001]')"/>
            </div>
        </xsl:if>
        
        <!-- Oxygen's original time stamp -->
        <!--
        <xsl:if test="oxyf:getParameter('webhelp.footer.add.generation.time') = 'yes'">
            <div class="generation_time">
                Generation date: <xsl:value-of select="format-dateTime(current-dateTime(), '[h1]:[m01] [P] on [D01]/[M01]/[Y0001]')"/>
            </div>
        </xsl:if>
        -->
    </xsl:template>
